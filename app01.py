...
class DLApp(Application):
    ...
    def modified_all_atoms(self, ctl: Control):
        modified_facts_program = ""
        for atom in ctl.symbolic_atoms.by_signature("all", 5):
            if atom.is_fact:
                args = atom.symbol.arguments
                param4_modified = args[3].number * 2
                modified_fact = f"all({args[0].number},{args[1]},{args[2].number},{param4_modified},{args[4].number}).\n"
                modified_facts_program += modified_fact

        print("Modified facts as ASP program:")
        print(modified_facts_program)
        return modified_facts_program
        
        
    def check_degradation(self):
        value = 0 if random.random() < 0 else 1
        if value == 1:
            print("There is degradation")
            degradation = True
        else:
            print("There is no degradation")
            degradation = False
        return degradation
    ...
    def main(self, ctl: Control, files: Sequence[str]):
        ...
        #ctl.load("encoding_criticality_seq.lp")
        ctl.add("base", [], THEORY)
        ...
        if not degradation:
            ...
            #self.modifying_protime(ctl)
            new_facts = self.modified_all_atoms(ctl)
            
            ctl_new = Control()
            ctl_new.register_propagator(self._propagator)
            ctl_new.add("base", [], THEORY)
            
            ctl_new.load("encoding_criticality_seq.lp")
            ctl_new.add("base", [], new_facts)
            
            ctl_new.ground([("base", [])])
            ctl_new.solve(on_model=self._propagator.on_model)
            
            
        #facts = self.get_facts(ctl)
        #self.new_processing_time(facts)
